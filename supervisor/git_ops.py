...
def ensure_repo_present() -> None:
    # Skip git operations when OUROBOROS_NO_GIT_SYNC is set
    if os.environ.get("OUROBOROS_NO_GIT_SYNC", "").lower() in ("1", "true", "yes"):
        return
...
def _create_rescue_snapshot(branch: str, reason: str, repo_state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if rc_status == 0:
        atomic_write_text(rescue_dir / "status.porcelain.txt", status_txt + ("\n" if status_txt else ""))

    rc_diff, diff_txt, diff_err = git_capture(["git", "diff", "--binary", "HEAD"])
    if rc_diff == 0:
        atomic_write_text(rescue_dir / "changes.diff", diff_txt + ("\n" if diff_txt else ""))
    else:
    ...
    atomic_write_text(rescue_dir / "rescue_meta.json", json.dumps(info, ensure_ascii=False, indent=2))
    return info
