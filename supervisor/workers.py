...
def auto_resume_after_restart() -> None:
    ...
    """
    log.info("[auto-resume] >>> auto_resume_after_restart() called")
    try:
        st = load_state()
        chat_id = st.get("owner_chat_id")
        log.info(f"[auto-resume] owner_chat_id from state: {chat_id}")
        if not chat_id:
            log.info("[auto-resume] No owner_chat_id - exiting early")
        ...
        log.info(f"[auto-resume] Proceeding with auto-resume for chat_id={chat_id}")
        ...
        def _do_resume():
            try:
                log.info("[auto-resume] _do_resume: Starting")
                time.sleep(3)
                log.info("[auto-resume] _do_resume: Creating task")
                ...
                log.info("[auto-resume] _do_resume: Calling assign_tasks()")
                assign_tasks()
                log.info("[auto-resume] _do_resume: assign_tasks() completed")
                log.info(f"Auto-resume: assign_tasks() called, workers should now pick up task")
            except Exception as e:
                log.error(f"[auto-resume] _do_resume FAILED: {e}")
