...
def log_skill_outcome(
    ...
    Records what happened AFTER a skill was used, enabling:
    - Success rate calculation per skill
    - Best skill selection based on past performance
    - Institutional memory of what works

    Args:
        skill_name: Name of the skill used
        task: Task description (truncated for storage)
        success: Whether the task completed successfully
        score: Quality score (0.0-1.0)
        metadata: Additional context (files changed, rounds, etc.)
    """
    outcome_entry = {
        "timestamp": datetime.now().isoformat(),
...
def get_skill_success_rates() -> Dict[str, Dict[str, float]]:
    ...
    Returns:
        Dict mapping skill_name to:
        - success_rate: Percentage of successful activations
        - avg_score: Average quality score
        - total_uses: Total number of outcomes recorded
...
def get_best_skill_for_task(task: str, available_skills: Optional[List[str]] = None) -> Optional[str]:
    ...
    Uses keyword matching + historical success rates to select skills.
    Skills with higher success rates on similar tasks are preferred.

    Args:
        task: Task description
        available_skills: Optional list of skill names to consider

    Returns:
        Best skill name or None if no good match found
    """
    if not task:
        return None
    ...
    # Score skills by keyword match + historical performance
    ...
    for skill_name, skill in SKILLS.items():
        ...
        # Skip if no keyword match at all
        if keyword_score == 0:
            continue

        # Historical performance bonus (0-100 scale)
        history_bonus = 0
        if skill_name in success_rates:
            rate = success_rates[skill_name]
            # Weight by success rate and usage count
            history_bonus = rate["success_rate"] * 50  # Max 50 points from success rate
            # Bonus for frequently used skills (more reliable data)
            if rate["total_uses"] >= 5:
                history_bonus += 10  # Reliability bonus
            if rate["total_uses"] >= 10:
                history_bonus += 10  # More reliability bonus
...
def export_skill_outcomes_to_vault(repo_dir: Optional[Path] = None) -> str:
    ...
    Creates/updates vault/concepts/skill_outcomes.md with:
    - Success rates per skill
    - Skills that work well
    - Skills that need improvement

    This provides persistent learning across sessions.
    """
    if repo_dir is None:
    ...
    try:
        ...
        if needs_work:
            lines.append("### ⚠️ Skills Needing Improvement")
            lines.append("")
