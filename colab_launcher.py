...
def main() -> None:
    ...
    # 5) Bootstrap repo
    # ----------------------------
    # Skip git sync when running locally
    if os.environ.get("OUROBOROS_NO_GIT_SYNC", "").lower() not in ("1", "true", "yes"):
        ensure_repo_present()
        ok, msg = safe_restart(reason="bootstrap", unsynced_policy="rescue_and_reset")
        assert ok, f"Bootstrap failed: {msg}"
    else:
        print("[boot] Skipping git sync (OUROBOROS_NO_GIT_SYNC=1)")
        ...
        for upd in updates:
            ...
            if agent._busy:
                ...
                if agent._busy:
                    ...
                    if _batched_image:
                        send_with_budget(
                            chat_id, "📎 Photo received, but a task is in progress. Send again when I'm free."
                        )
                else:
